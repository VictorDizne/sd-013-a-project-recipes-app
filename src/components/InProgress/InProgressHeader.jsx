...
function InProgressHeader({ spec }) {
  ...
  const { protocol, host } = window.location; // constantes que serão usadas para copiar a URL;
  ...
  const shareRecipe = () => { // Função que copia o link (URL) da receita para o clipboard;
    copy(`${protocol}//${host}/${page}/${id}`);
    const copyMsg = document.querySelector('.InProgress-copyMsg');
  ...
  const toggleFav = () => { // Função que coloca ou tira a receita dos favoritos;
    if (isFavorite) {
      ...
      const newFav = newFavObject(recipe, spec); // Se não estiver favoritada, monta o objeto com os dados da receita;
  ...
  return (
    <div className="InProgress-header-container">
      ...
      <div className="InProgress-header-buttons">
        <span className="InProgress-copyMsg invisible">Link copiado!</span>
